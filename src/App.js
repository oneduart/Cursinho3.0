...
const AuthScreen = ({ onLogin }) => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.message.includes("auth") ? "Erro de email ou senha." : err.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 bg-slate-100 flex flex-col animate-fadeIn">
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-2xl w-full text-center border-4 border-blue-100">
          <span className="uppercase text-xs font-bold text-blue-400 tracking-widest mb-4 block">{slide.type}</span>
          <h1 className="text-3xl font-bold mb-6">{slide.title}</h1>
          <div className="space-y-4 text-lg text-left inline-block">
            {slide.content.map((t, i) => <p key={i} className="flex gap-2"><ArrowRight size={20} className="text-blue-500 shrink-0"/> {t}</p>)}
          </div>
          <button onClick={handleNext} className="mt-8 w-full bg-blue-600 text-white py-3 rounded-xl font-bold hover:scale-105 transition-transform">
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20">
      ...
      <div className="bg-white p-4 shadow-sm flex justify-between items-center sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-bold text-sm leading-tight">{userData?.name || "Aluno"}</h1>
        ...
        {userData?.role === 'teacher' && (
          <div className="flex gap-2 mb-6 bg-gray-200 p-1 rounded-lg">
            <button 
              ...
              className={`flex-1 py-2 text-sm font-bold rounded-md ${view === 'modules' ? 'bg-white shadow' : 'text-gray-600'}`}
            >
              Ver Curso
            </button>
            <button 
              onClick={() => setView('teacher')}
              className={`flex-1 py-2 text-sm font-bold rounded-md ${view === 'teacher' ? 'bg-white shadow' : 'text-gray-600'}`}
            >
              Área do Professor
            </button>
          </div>
          ...
          <div className="animate-fadeIn">
            <div className="flex justify-between items-end mb-4">
              ...
              <span className="text-xs bg-blue-100 text-blue-700 px-2 py-1 rounded-full font-bold">
                {userData?.completedModules?.length || 0}/{courseData.modules.length}
              </span>
            </div>
